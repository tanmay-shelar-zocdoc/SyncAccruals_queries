...
FROM 
  ...
  INNER JOIN MostRecentPMS P on P.SyncDBID = SLMH.SyncDBID and P.pms_id = 10 
...
ActivePracticesInRange AS 
 ...
 FROM provider_analytics.practice_snapshot_summary_month PSM 
      INNER JOIN provider.practice PT on PT.PRACTICE_ID = PSM.practice_id
 WHERE PSM.SNAPSHOT_MONTH_EASTERN = $MonthToReport
 AND PSM.days_in_month_active > 0),
...
SyncedSourceLocations as
    (
        ...
        FROM provider_raw.aggregate_location_mapping_update U
                 ...
                 INNER JOIN ActiveDentrixSyncsInRange DSY on DSY.cloud_location_id = U.source_location_id
...
FROM 
   ...
   INNER JOIN ActiveDentrixSyncsInRange DS on DS.cloud_practice_id = AP.practice_id
