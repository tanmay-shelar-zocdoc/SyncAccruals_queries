...
FROM LastPMS
      ...
      left join (
          ...
          FROM SyncLocationMappingHistory SLMH
               INNER JOIN LastPMS LP on LP.SyncDBID = SLMH.SyncDBID and LP.pms_application = 'AllScriptsAPI'
...
WHERE LastPMS.pms_application IN ('AllScriptsAPI')
GROUP BY 1,2,3,4
ORDER BY initial_synced_date DESC)

SELECT * 
FROM AllscriptsAPIPractices
