...
ActiveEaglesoftSyncsInRange AS 
(SELECT DISTINCT 
    SLMH.SyncDBID,
    SLMH.LocationID,
...
FROM 
  ...
  INNER JOIN MostRecentPMS P on P.SyncDBID = SLMH.SyncDBID and P.pms_id in (11,12)
...
SyncedSourceLocations as
    (
        ...
        FROM provider_raw.aggregate_location_mapping_update U
                 ...
                 INNER JOIN ActiveEaglesoftSyncsInRange DSY on DSY.cloud_location_id = U.source_location_id
...
FROM 
   ...
   INNER JOIN ActiveEaglesoftSyncsInRange DS on DS.cloud_practice_id = AP.practice_id
