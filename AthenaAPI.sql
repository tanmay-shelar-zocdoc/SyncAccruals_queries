...
SELECT DISTINCT PT.practice_id, 
             ...
             count(distinct StillSyncedToAthena.LocationID) as num_locations_still_synced,
...
FROM LastPMS
      ...
      left join (
          ...
          FROM SyncLocationMappingHistory SLMH
               INNER JOIN LastPMS LP on LP.SyncDBID = SLMH.SyncDBID and LP.pms_application IN ('Athena','AthenaAPI')
          WHERE deleted_date_utc IS NULL
    ) StillSyncedToAthena on StillSyncedToAthena.LocationID = L.monolith_provider_location_id
WHERE LastPMS.pms_application IN ('Athena','AthenaAPI')
GROUP BY 1,2,3,4
ORDER BY initial_synced_date DESC)

SELECT * 
FROM AthenaAPIPractices
